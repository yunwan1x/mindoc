...
WHERE book.privately_owned = 0 AND (doc.document_name LIKE ? OR doc.content LIKE ?) `
...
WHERE (book.privately_owned = 0 OR rel1.relationship_id > 0 or team.team_member_id > 0)  AND (doc.document_name LIKE ? OR doc.content LIKE ?);`
...
FROM (
       ...
       WHERE (book.privately_owned = 0 OR rel1.relationship_id > 0 OR team.team_member_id > 0) AND
             (doc.document_name LIKE ? OR doc.content LIKE ?)
       UNION ALL
...
func (m *DocumentSearchResult) FindByLabelToPager(label string, pageIndex, pageSize, memberId int) (searchResult []*DocumentSearchResult, totalCount int, err error) {
	o := orm.NewOrm()
	offset := (pageIndex - 1) * pageSize
...
WHERE label.label_name =  ? AND relation.relation_type = 'book'
       ...
       FROM md_blogs AS blog
         LEFT JOIN md_members AS member ON blog.member_id = member.member_id
LEFT JOIN md_label_relation AS relation ON relation.resource_id = blog.blog_id
		LEFT JOIN md_label AS label ON label.label_id = relation.label_id
       WHERE blog.blog_status = 'public' AND (label.label_name =  ?) AND relation.relation_type = 'blog'
     ) AS union_table
ORDER BY create_time DESC
LIMIT ?, ?;`

		err = o.Raw(sql1, label).QueryRow(&totalCount)
		if err != nil {
			logs.Error("查询搜索结果失败 -> ", err)
			return
		}
...
`
		...
		c := 0
		err = o.Raw(sql3, label).QueryRow(&c)
		if err != nil {
			logs.Error("查询搜索结果失败 -> ", err)
			return
		}
...
WHERE label.label_name =  ? AND relation.relation_type = 'book'`
		...
		_, err = o.Raw(sql2, label, label, label, offset, pageSize).QueryRows(&searchResult)
		if err != nil {
			logs.Error("查询搜索结果失败 -> ", err)
			return
		}
...
LEFT JOIN md_label_relation AS relation ON relation.resource_id = doc.document_id
		LEFT JOIN md_label AS label ON label.label_id = relation.label_id
WHERE (book.privately_owned = 0 OR rel1.relationship_id > 0 or team.team_member_id > 0)  AND  (label.label_name =  ?) AND relation.relation_type = 'doc';`
...
FROM (
       ...
       WHERE (book.privately_owned = 0 OR rel1.relationship_id > 0 OR team.team_member_id > 0) 
             AND (label.label_name =  ?) AND relation.relation_type = 'doc'
       UNION ALL
       ...
       FROM md_books AS book
		...
		LEFT JOIN md_label AS label ON label.label_id = relation.label_id
       WHERE (book.privately_owned = 0 OR rel1.relationship_id > 0 OR team.team_member_id > 0) 
		AND (label.label_name =  ?) AND relation.relation_type = 'book'
 UNION ALL
       SELECT
         blog.blog_id AS document_id, 
         blog.modify_time,
	...
	LEFT JOIN md_label_relation AS relation ON relation.resource_id = blog.blog_id
       ...
       WHERE (blog.blog_status = 'public' OR blog.member_id = ?) AND blog.blog_type = 0 
		AND (label.label_name =  ?) AND relation.relation_type = 'blog'
     ) AS union_table
ORDER BY create_time DESC
LIMIT ?, ?;`

		err = o.Raw(sql1, memberId, memberId, label).QueryRow(&totalCount)
		if err != nil {
			return
		}
		sql3 := `       SELECT
...
`
		...
		c := 0
		err = o.Raw(sql3, memberId, label).QueryRow(&c)
		if err != nil {
			logs.Error("查询搜索结果失败 -> ", err)
			return
		}
...
`
		...
		err = o.Raw(sql4, memberId, memberId, label).QueryRow(&c)
		if err != nil {
			logs.Error("查询搜索结果失败 -> ", err)
			return
		}
		...
		_, err = o.Raw(sql2, memberId, memberId, label, memberId, memberId, label, memberId, label, offset, pageSize).QueryRows(&searchResult)
		if err != nil {
			return
		}
	}
